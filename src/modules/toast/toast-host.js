...
class ToastHostElement extends LitElement {
  ...
  disconnectedCallback() {
    ...
    this._dismissTimers.forEach((timerId) => window.clearTimeout(timerId));
    this._dismissTimers.clear();
    this._expirationTimers.forEach((timerId) => window.clearTimeout(timerId));
    this._expirationTimers.clear();
    this._visibilityFrames.forEach((frameId) => window.cancelAnimationFrame(frameId));
  ...
  _showNextStandardToast() {
    if (this._activeStandardEntry || this._undoEntries.length > 0 || this._standardQueue.length === 0) {
      this.requestUpdate();
      return;
    }
  ...
  _clearAnnouncementFrame(type) {
    const frameKey = type === 'assertive'
      ? '_assertiveAnnouncementFrame'
      : '_politeAnnouncementFrame';
  ...
  _announce(entry) {
    const announcementKey = entry.type === 'undo'
      ? '_assertiveAnnouncement'
      : '_politeAnnouncement';
    const frameKey = entry.type === 'undo'
      ? '_assertiveAnnouncementFrame'
      : '_politeAnnouncementFrame';
