...
for (const selector of selectors) {
  traces[selector] = traceExtraction(
    decoded.data,
    decoded.width,
    decoded.height,
    SWATCH_COUNT,
...
for (const selector of selectors) {
  ...
  for (let i = 0; i < sel.length; i++) {
    ...
    console.log(
      `  #${i + 1} ${hex(s.rgb)} rgb(${s.rgb.r},${s.rgb.g},${s.rgb.b}) L=${lab.L.toFixed(2)} chroma=${c.toFixed(3)} ${hueName(lab)}`,
    );
  }
}
...
for (const selector of selectors) {
  ...
  for (let i = 0; i < Math.min(sorted.length, 20); i++) {
    ...
    console.log(
      `  ${hex(c.rgb)} L=${lab.L.toFixed(2)} chroma=${chroma.toFixed(3)} mass=${c.population ?? 0} ${hueName(lab)}`,
    );
  }
}

console.log("\n--- hybrid internals: why each pick was made ---");
const hybridResult = selectPaletteHybrid(
  decoded.data,
  decoded.width,
  decoded.height,
  SWATCH_COUNT,
...
const hybCands = hybridResult.candidates
  .map((c, i) => ({
    i,
    ...c,
    lab: rgbToOklab(c.meanRgb.r, c.meanRgb.g, c.meanRgb.b),
    vividLab: rgbToOklab(c.vividRgb.r, c.vividRgb.g, c.vividRgb.b),
  }))
...
for (const c of hybCands) {
  ...
  console.log(
    `  [#${c.i}] mean=${hex(c.meanRgb)} L=${c.lab.L.toFixed(2)} chroma=${meanChroma.toFixed(3)} | vivid=${hex(c.vividRgb)} chroma=${vividChroma.toFixed(3)} | mass=${c.mass} ${hueName(c.lab)}`,
  );
}
